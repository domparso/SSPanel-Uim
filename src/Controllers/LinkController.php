...
final class LinkController extends BaseController
{
    public static function getContent(ServerRequest $request, Response $response, array $args): Response|ResponseInterface
    {
        ...
        if (isset($params['sr']) && $params['sr'] === '1') {
            $sub_type = 'sr';
            $sub_info = self::getSR($user);
        }
    ...
    // ios Shadowrocket 订阅
    public static function getSR($user): string
    {
        $links = '';
        //判断是否开启V2Ray订阅
    ...
    public static function getV2Ray($user): string
    {
        ...
        $links .= 'vmess://' . base64_encode(json_encode($v2rayn_array1,JSON_UNESCAPED_SLASHES)) . PHP_EOL;
        $v2rayn_array2 = [
            'v' => '2',
            'ps' => $expire_in,
            'add' => $baseUrl,
        ...
        $links .= 'vmess://' . base64_encode(json_encode($v2rayn_array2,JSON_UNESCAPED_SLASHES)) . PHP_EOL;
